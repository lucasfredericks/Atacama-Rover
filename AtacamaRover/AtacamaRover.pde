...
void setup() {
  ...
  background(150);
  text("setup complete", width/2, height/2);
}


void initArena() {
  background(150);
  text("searching for camera", width/2, height/2);
  while (true) {
    if (cam.available()) {
      cam.read();
      background(150);
      text("camera found", width/2, height/2);
    }
    background(150);
    text("searching for computer vision markers", width/2, height/2);
    int numCorners = 7;
    if (cvThread.dataFlag) {
      ...
      for ( FiducialFound f : found ) {
        if ((int)f.getId()==1234) {
          background(150);
          text("arena found", width/2, height/2);
          arena = new Arena();
          ...
          Point3D_F64 [] rwcorners = { //rw coordinates of arena corners in centimeters, with CV marker at (0,0)
            new Point3D_F64(-150, 170, 0), 
            new Point3D_F64(-20, 170, 0), 
            new Point3D_F64(170, 140, 0), 
            new Point3D_F64(180, 60, 0), 
            new Point3D_F64(100, -80, 0), 
            new Point3D_F64(-90, -140, 0), 
            new Point3D_F64(-150, 0, 0), 
